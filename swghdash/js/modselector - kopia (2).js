...
// Updated: 2026-03-07 23:55
// Description: Core logic for the Mod Selector Overlay. 
// Handles reverse search with strict deficit check (Equipped < Recommended).
            ...
            if (showEq) {
                ...
                if (match && targetSets) {
                    ...
                    for (const sName of targetSets) {
                        ...
                        // If not recommended for this unit at all, it's not a match
                        if (!rItem) { match = false; break; }

                        const eItem = eqCounts.find(e => normalize(e.type) === sName);
                        const eVal = eItem ? eItem.count : 0;
                        
                        // If equipped >= recommended, the deficit is 0. Hide unit.
                        if (eVal >= rItem.count) { match = false; break; }
                    }
                }
                if (match && targetPrimary && targetShape) { 
                    const recP = normalize(u[targetShape + "(r)"]); 
