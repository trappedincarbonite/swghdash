...
// Updated: 2026-03-07 10:30:00
// Description: Core logic for the Mod Selector Overlay. 
// This file handles the reverse search logic, mod set parsing, and table rendering.
...
window.renderTable = function() {
    ...
    units = units.filter(u => {
        ...
        if (window.selection.shape || window.selection.sets.length > 0 || window.selection.primary) {
            ...
            if (showEq) {
                ...
                if (match && targetSets) { const recSet = normalize(u["set(r)"]); if (!targetSets.every(s => recSet.includes(s))) match = false; }
                if (match && targetPrimary && targetShape) { const recP = normalize(u[targetShape + "(r)"]); if (!recP.includes(targetPrimary)) match = false; }
            }
        }
        return match;
    });
    ...
    // Handle external stats calculation safely
    if (typeof calculateGameStats === 'function') {
        ...
        if (typeof renderCentralFooter === 'function') renderCentralFooter(stats);
    }
    ...
    tbody.innerHTML = units.map(u => {
        ...
        const getCell = (slot) => {
            ...
            if (!isM(eq)) {
                ...
                return `<td class="col-${slot}"><div class="cell-flex"><span class="mod-badge ${statusClass} ${highlight}">${dE}</span>${!isPerf ? `<span class="mod-rec-hint">${dR}</span>` : ''}</div></td>`;
            }
            return `<td class="col-${slot}"><div class="cell-flex"><span class="mod-badge ${dR === 'N/A' ? 'mod-none' : 'mod-ghost'} ${highlight}">${dR}</span></div></td>`;
        };
...
window.getSetCell = function(u) {
    ...
    if (recSets.length === 0) return `<td class="col-set"><div class="cell-flex"><span class="mod-badge mod-none">N/A</span></div></td>`;
    let html = [];
...
window.renderHeader = function() {
    ...
    if (!row) return;
    const cols = [
        { label: 'Unit Name', key: 'name', cls: 'col-name' }, { label: 'Lvl', key: 'level', cls: 'col-lvl' },
    ...
    row.innerHTML = cols.map(c => {
        ...
        const borderClass = (c.key === 'gear' || c.key === 'set') ? 'border-r border-slate-800/40' : '';
        const paddingClass = c.key === 'name' ? 'pl-6' : '';
        return `<th class="${c.cls} text-left py-3 px-3 uppercase text-[11px] font-bold tracking-wide border-b border-slate-800 ${paddingClass} ${borderClass}" onclick="window.handleHeaderClick('${c.key}')">${c.label}${arrow}</th>`;
    }).join('');
};